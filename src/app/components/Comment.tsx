...
const Comment: React.FC<CommentProps> = ({ comment: initialComment, onReply, onEdit, onDelete, depth = 0, clerkUserId }) => {
  ...
  const handleReply = async () => {
    if (comment.id && canReply) {
      const parentUserId = comment?.replies?.[0]?.parent?.userId as string
      await onReply(comment.id, replyBody, parentUserId);
      const newReply: iComment = {
        ...
        parentUserId: comment?.replies?.[0]?.parent?.userId,
      };
        ...
        <div className="flex-1 min-w-0">
          ...
          {isReplying && (
            <div className="mt-2">
              ...
              />
              <Button className="mt-2 bg-blue-500 text-white text-xs px-2 py-1 h-6" onClick={handleReply}>
                Submit Reply
              </Button>
            </div>
          )}
